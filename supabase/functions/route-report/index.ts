...
const POLICY_VERSION = 'NO_FINANCIAL_DISCLOSURE_V1';
const FINANCIAL_KEY = /(show.?cost|cost|price|pricing|tco|amount|fee|quote|quotation|revenue|profit|margin|discount|budget|invoice|freight.?rate|unit.?rate)/i;
const FINANCIAL_LABEL = /(TCO|成本|价格|单价|报价|费用|金额|运费|利润|毛利|折扣|预算|账单|人民币|美元|元\/柜|CNY\s*\/|USD\s*\/)/i;
const FINANCIAL_VALUE = /((TCO|成本|价格|单价|报价|费用|金额|运费|利润|毛利|折扣|预算|cost|price|quote|fee|amount|freight)[^。；,\n]{0,24}(¥|￥|\$|USD|CNY|RMB|\d[\d,.]*))|((¥|￥|\$|USD|CNY|RMB)\s*\d)|(\d[\d,.]*\s*(元|万元|美元|人民币)(\/柜|每柜)?)/i;
...
function stripFinancials(value:any):any{
  if(Array.isArray(value))return value.map(stripFinancials).filter(v=>v!==undefined);
  if(value&&typeof value==='object'){
    const marker=[value.key,value.label].filter(Boolean).join(' ');
    if(marker&&FINANCIAL_LABEL.test(marker))return undefined;
    const out:AnyRow={};
    for(const [key,item] of Object.entries(value)){
      if(FINANCIAL_KEY.test(key))continue;
      const safe=stripFinancials(item);if(safe!==undefined)out[key]=safe;
    }
    return out;
  }
  if(typeof value==='string'&&FINANCIAL_VALUE.test(value))return '【敏感财务内容已移除】';
  return value;
}
function sanitizeSnapshot(input:any){
  const legacyFinancial=!!input?.governance?.legacy_financial_model_sanitized||input?.show_cost===true||input?.comparison?.rows?.some((g:any)=>FINANCIAL_LABEL.test([g?.key,g?.label].join(' ')));
  const out=stripFinancials(input)||{};
  out.governance={...(out.governance||{}),policy_version:POLICY_VERSION,sensitive_financials_excluded:true,financial_disclosure:'prohibited'};
  if(legacyFinancial){
    if(out.comparison){out.comparison.scores=[];out.comparison.leader_index=null;out.comparison.summary='本历史快照原含财务维度，现已按敏感数据策略移除相关数值及其派生结论；请重新生成新版公开安全报告。';out.comparison.recommendation='请使用当前公开安全模型重新生成报告后，再形成对外结论。'}
    out.governance.legacy_financial_model_sanitized=true;
  }
  return out;
}
...
function publicModel(model:AnyRow){
  const config=model?.config||{},allowed=GROUPS.flatMap(g=>g.metrics.map((m:any)=>m[0])),raw=config.weights||{},total=allowed.reduce((sum,key)=>sum+Math.max(0,Number(raw[key]||0)),0)||1;
  const weights:AnyRow={};let assigned=0;
  allowed.forEach((key,index)=>{const value=index===allowed.length-1?Number((100-assigned).toFixed(2)):Number((Math.max(0,Number(raw[key]||0))/total*100).toFixed(2));weights[key]=value;assigned=Number((assigned+value).toFixed(2))});
  const thresholds:AnyRow={};for(const key of allowed)thresholds[key]=config.materiality_thresholds?.[key]??0;
  return {code:model.code,label:model.label,config:{...config,weights,weight_total:100,materiality_thresholds:thresholds,public_safe_policy:POLICY_VERSION}};
}
...
function compareRoutes(routes:AnyRow[],model:AnyRow){
  ...
  if(readinessIndex>=65)recommendation=leader?(leader.index===jmIndex?'建议先安排1柜江门试运行，用实测分段时效、温控和通关结果校准模型。':'当前不建议整体切换江门；可针对江门短板补强后，以1柜进行同口径对照试运行。'):'建议保持现有方案，同时用1柜对照试运行验证差异是否具有业务意义。';
...
function validateRoutes(routes:AnyRow[]){
  ...
  for(const r of routes){
    ...
    const p50=(r.segments||[]).reduce((a:number,s:any)=>a+(finite(s.p50_days)??0),0),p90=(r.segments||[]).reduce((a:number,s:any)=>a+(finite(s.p90_days)??0),0);
    const p50Delta=finite(r.metrics.p50_days)===null?null:Number((p50-Number(r.metrics.p50_days)).toFixed(2)),p90Delta=finite(r.metrics.p90_days)===null?null:Number((p90-Number(r.metrics.p90_days)).toFixed(2));
    reconciliation.push({route_id:r.id,route_name:r.name,segment_count:(r.segments||[]).length,p50_sum:Number(p50.toFixed(2)),p90_sum:Number(p90.toFixed(2)),p50_delta:p50Delta,p90_delta:p90Delta});
...
async function loadReportRoutes(currentIds:string[],jiangmenId:string){
  ...
  const routes=currentRows.map((r:any)=>routeFromCurrent(r,pickPortStat(String(r.transport_mode||'').includes('陆运')?landStats:seaStats,r,r.current_port),forRoute('current',r.id)));
  routes.push(routeFromJiangmen(jm,pickPortStat(jmStats,jm,'江门',true),forRoute('jiangmen',jm.id)));
...
async function createReport(b:AnyRow,member:any){
  ...
  const {routes,currentRows,jm}=await loadReportRoutes(ids,jiangmenId),model=publicModel(await activeModel()),validation=validateRoutes(routes),comparison=compareRoutes(routes,model),{customer,opportunity}=await customerAndOpportunity(b,member);
  ...
  const eligible=validation.publish_blockers.length===0,status=eligible?'published':'draft',expiresAt=new Date(Date.now()+days*86400000).toISOString();
  let snapshot:any={schema_version:3,generated_method:'deterministic',model_version:model.code,model:{code:model.code,label:model.label,method:model.config?.method,weight_total:model.config?.weight_total,weights:model.config?.weights,materiality_thresholds:model.config?.materiality_thresholds,public_safe_policy:POLICY_VERSION},is_demo:isDemo,report_number:reportNo,title:text(b.title||'FONKON供应链路线决策报告',120),client_name:b.show_client_name===false?'贵司':clientName,created_at:createdAt,expires_at:expiresAt,requires_code:!!accessCode,prepared_by:{name:text(member.profile.name||'FONKON顾问',80),email:text(member.profile.email||'',120)},approval:{status,publication_mode:'automatic',submitted_at:null,approved_at:null,approved_by:null,published_at:eligible?createdAt:null},governance:{publication_mode:'automatic',policy_version:POLICY_VERSION,decision:eligible?'pass':'blocked',generated_by:{id:member.user.id,name:text(member.profile.name,80),email:text(member.profile.email,120)},generated_at:createdAt,sensitive_financials_excluded:true,financial_disclosure:'prohibited'},scope,assumptions:text(b.assumptions||'同起点、同终点、同水果、同一业务月份、同币种、同柜型；未知数据不按0计算。',600),routes,comparison,validation,sources:sourceManifest(routes),disclaimer:isDemo?'本报告全部使用系统模拟数据，仅用于功能演示与流程体验，不代表真实港口运营表现，不得作为对外业务决策依据；报告不包含任何成本或价格金额。':'本报告由固定规则模型根据生成时已复核的同口径数据计算，未知值不按0计算，且不包含任何成本或价格金额；正式切换前仍建议通过试柜复核。'};
  snapshot=sanitizeSnapshot(snapshot);const snapshotHash=eligible?await hashText(canonical(snapshot)):null;
  const decision={result:eligible?'pass':'blocked',blocker_count:validation.publish_blockers.length,blockers:validation.publish_blockers,public_safe:true,sensitive_financials_excluded:true};
  const row={id,report_number:reportNo,title:snapshot.title,customer_id:customer?.id||null,opportunity_id:opportunity?.id||null,created_by:member.user.id,status,model_version:model.code,is_demo:isDemo,share_token:shareToken,access_code_hash:accessCode?await hashText(id+':'+accessCode):null,expires_at:expiresAt,published_at:eligible?createdAt:null,snapshot_hash:snapshotHash,publication_mode:'automatic',policy_version:POLICY_VERSION,policy_decision:decision,snapshot};
  await requestJson('/rest/v1/route_reports',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify(row)});await event(id,'create',member.user.id,{model_version:model.code,is_demo:isDemo,blocker_count:validation.publish_blockers.length,policy_version:POLICY_VERSION});
  if(eligible)await event(id,'auto_publish',member.user.id,{snapshot_hash:snapshotHash,policy_version:POLICY_VERSION,decision:'pass'});
  return {ok:true,id,report_number:reportNo,status,is_demo:isDemo,model_version:model.code,readiness_index:comparison.data_readiness.index,publish_blockers:validation.publish_blockers,share_token:eligible?shareToken:null,expires_at:expiresAt,publication_mode:'automatic',policy_version:POLICY_VERSION};
}
...
async function workflow(action:string,b:AnyRow,member:any){
  const report=await assertReportAccess(text(b.report_id,40),member),now=isoNow(),snapshot=sanitizeSnapshot(report.snapshot||{}),approval={...(snapshot.approval||{})};
  ...
  if(action==='approve_publish'){
    ...
    approval.status='published';approval.publication_mode='manual_legacy';approval.approved_at=now;approval.approved_by={name:text(member.profile.name,80),email:text(member.profile.email,120)};snapshot.approval=approval;snapshot.governance={...(snapshot.governance||{}),publication_mode:'manual_legacy',policy_version:POLICY_VERSION,decision:'approved',sensitive_financials_excluded:true};const snapshotHash=await hashText(canonical(snapshot));
    await requestJson('/rest/v1/route_reports?id=eq.'+queryValue(report.id),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status:'published',approved_at:now,published_at:now,approved_by:member.user.id,snapshot_hash:snapshotHash,review_notes:text(b.review_notes,500)||null,publication_mode:'manual_legacy',policy_version:POLICY_VERSION,policy_decision:{result:'approved',public_safe:true,sensitive_financials_excluded:true},snapshot,updated_at:now})});await event(report.id,'approve_publish',member.user.id,{snapshot_hash:snapshotHash,review_notes:text(b.review_notes,500)||null,policy_version:POLICY_VERSION});return {ok:true,status:'published',share_token:report.share_token,expires_at:report.expires_at};
  }
...
async function previewReport(b:AnyRow,member:any){
  const report=await assertReportAccess(text(b.report_id,40),member),snapshot=sanitizeSnapshot(report.snapshot||{});
  await event(report.id,'preview',member.user.id,{status:report.status,policy_version:report.policy_version||POLICY_VERSION});
  return {ok:true,id:report.id,report_number:report.report_number,status:report.status,created_at:report.created_at,published_at:report.published_at,expires_at:report.expires_at,publication_mode:report.publication_mode||'manual_legacy',policy_version:report.policy_version||POLICY_VERSION,policy_decision:report.policy_decision||{},snapshot};
}
...
async function saveSegments(b:AnyRow,member:any){
  ...
  const table=kind==='current'?'current_route_baselines':'jiangmen_routes',[parents,existing]=await Promise.all([requestJson('/rest/v1/'+table+'?id=eq.'+queryValue(routeId)+'&select=id,p50_days,p90_days,currency,container_type,is_demo'),requestJson('/rest/v1/route_chain_segments?route_kind=eq.'+queryValue(kind)+'&route_id=eq.'+queryValue(routeId)+'&select=segment_order,cost_amount')]),parent=parents?.[0];if(!parent)throw new Error('未找到路线');const existingByOrder=new Map(existing.map((s:any)=>[Number(s.segment_order),s]));
  const rows=input.map((s:any,i:number)=>{const p50=finite(s.p50_days),p90=finite(s.p90_days),risk=finite(s.coldchain_risk_index),source=text(s.public_source_text,300);if(p50===null||p90===null||p90<p50||risk===null||risk<0||risk>10||!source)throw new Error('第'+(i+1)+'段数据不完整或范围错误');return {route_kind:kind,route_id:routeId,segment_order:i+1,segment_code:SEGMENT_CODES[i],segment_name:text(s.name,100)||SEGMENT_LABELS[SEGMENT_CODES[i]],origin_node:text(s.origin,100)||null,destination_node:text(s.destination,100)||null,p50_days:p50,p90_days:p90,cost_amount:finite(existingByOrder.get(i+1)?.cost_amount),currency:text(parent.currency,3)||'CNY',container_type:text(parent.container_type,20)||'40RF',coldchain_risk_index:risk,evidence_level:['A','B','C','D'].includes(text(s.evidence_level,1))?text(s.evidence_level,1):'C',sample_size:Math.max(0,Math.round(finite(s.sample_size)??0)),public_source_text:source,public_source_url:safeUrl(s.public_source_url),source_updated_at:s.source_updated_at||isoNow(),is_demo:!!parent.is_demo,updated_by:member.user.id,updated_at:isoNow()}});
  const sum=(k:string)=>rows.reduce((a:number,s:any)=>a+Number(s[k]||0),0),p50=sum('p50_days'),p90=sum('p90_days');if(finite(parent.p50_days)!==null&&Math.abs(p50-Number(parent.p50_days))>.05)throw new Error('9段P50合计必须等于路线P50 '+parent.p50_days+'天');if(finite(parent.p90_days)!==null&&Math.abs(p90-Number(parent.p90_days))>.05)throw new Error('9段P90合计必须等于路线P90 '+parent.p90_days+'天');
  await requestJson('/rest/v1/route_chain_segments?on_conflict=route_kind,route_id,segment_order',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(rows)});return {ok:true,segment_count:9,p50_total:p50,p90_total:p90};
}
async function publicReport(req:Request,url:URL){
  ...
  const safeSnapshot=sanitizeSnapshot(report.snapshot||{});
  return json({ok:true,report:{...safeSnapshot,is_demo:!!report.is_demo,model_version:report.model_version,expires_at:report.expires_at,integrity:{snapshot_hash:report.snapshot_hash,approved_at:report.approved_at,published_at:report.published_at,policy_version:POLICY_VERSION,sensitive_financials_excluded:true}}});
}
async function publicQr(url:URL){
  const token=text(url.searchParams.get('token'),100);if(!/^[A-Za-z0-9_-]{32,100}$/.test(token))return json({error:'分享链接无效'},404);const rows=await requestJson('/rest/v1/route_reports?share_token=eq.'+queryValue(token)+'&select=id,status,expires_at'),report=rows?.[0];if(!report||report.status!=='published'||(report.expires_at&&new Date(report.expires_at).getTime()<Date.now()))return json({error:'分享链接无效或已过期'},410);const shareUrl='https://os.fonkonsupply.com/report.html?v=20260824-r11-1-reports7&t='+encodeURIComponent(token),svg=await QRCode.toString(shareUrl,{type:'svg',width:240,margin:1,color:{dark:'#06283b',light:'#ffffff'}});return new Response(svg,{status:200,headers:{'access-control-allow-origin':'*','content-type':'image/svg+xml; charset=utf-8','cache-control':'private, max-age=300'}});
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(action==='preview')return json(await previewReport(b,member));
    if(action==='record_share'){
      const report=await assertReportAccess(text(b.report_id,40),member);if(report.status!=='published')throw new Error('只有正在分享的报告可以记录分享');
      await event(report.id,'share',member.user.id,{channel:text(b.channel,40)||'link_copy',policy_version:report.policy_version||POLICY_VERSION});return json({ok:true});
    }
    if(['submit_review','approve_publish','return_draft'].includes(action))return json(await workflow(action,b,member));
    if(action==='list'){
      const filter=isAdmin(member)?'':'&created_by=eq.'+queryValue(member.user.id),rows=await requestJson('/rest/v1/route_reports?select=id,report_number,title,status,model_version,is_demo,share_token,expires_at,view_count,unique_view_count,created_at,last_viewed_at,submitted_at,approved_at,published_at,review_notes,snapshot,created_by,publication_mode,policy_version,policy_decision&order=created_at.desc&limit=150'+filter);
      return json({ok:true,role:member.profile.role,can_approve:isAdmin(member),publication_mode:'automatic_policy_gate',policy_version:POLICY_VERSION,reports:rows.map((r:any)=>{const snapshot=sanitizeSnapshot(r.snapshot||{});return {id:r.id,report_number:r.report_number,title:r.title,status:r.status,model_version:r.model_version,is_demo:r.is_demo,share_token:r.status==='published'?r.share_token:null,expires_at:r.expires_at,view_count:r.view_count,unique_view_count:r.unique_view_count,created_at:r.created_at,last_viewed_at:r.last_viewed_at,submitted_at:r.submitted_at,approved_at:r.approved_at,published_at:r.published_at,review_notes:r.review_notes,client_name:snapshot.client_name||'客户',scope:snapshot.scope||{},route_count:snapshot.routes?.length||0,requires_code:!!snapshot.requires_code,readiness_index:snapshot.comparison?.data_readiness?.index??null,publish_blockers:snapshot.validation?.publish_blockers||[],publication_mode:r.publication_mode||snapshot.governance?.publication_mode||'manual_legacy',policy_version:r.policy_version||snapshot.governance?.policy_version||POLICY_VERSION,policy_decision:r.policy_decision||{}}})});
    }
    if(['revoke','renew','rotate'].includes(action)){
      ...
      if(action==='renew'){if(report.status!=='published')throw new Error('撤销或草稿报告不能直接续期');const rawDays=Number(b.expiry_days||30),days=Math.max(1,Math.min(365,Number.isFinite(rawDays)?rawDays:30)),expiresAt=new Date(Date.now()+days*86400000).toISOString();await requestJson('/rest/v1/route_reports?id=eq.'+queryValue(report.id),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({expires_at:expiresAt,updated_at:now})});await event(report.id,'renew',member.user.id,{expiry_days:days});return json({ok:true,expires_at:expiresAt,share_token:report.share_token})}
